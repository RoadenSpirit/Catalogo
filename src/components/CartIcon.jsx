    ...
    return (
        <div className="relative" ref={menuRef}> 
            <button 
                ...
                className="relative p-2 rounded-full text-white hover:bg-slate-700 transition duration-150 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
                ...
                {itemCount > 0 && (
                    <span 
                        className={twMerge(
                            "absolute top-0 right-0 inline-flex items-center justify-center w-6 h-6 text-xs font-bold leading-none text-white transform translate-x-1/2 -translate-y-1/2 bg-amber-500 rounded-full",
                            itemCount > 9 ? "p-1" : ""
                        )}
                    >
                        {itemCount}
            ...
            {isOpen && (
                <div className="absolute right-0 mt-3 w-80 bg-slate-800 border border-slate-700 rounded-lg shadow-2xl overflow-hidden z-30 transition-opacity duration-200">
                    <h4 className="px-4 py-3 bg-slate-700/50 text-amber-50 font-semibold border-b border-slate-700">
                        ...
                        <>
                                ...
                                {$cart.map(item => (
                                    <li key={item.id} className="flex items-center justify-between p-3 transition duration-150 hover:bg-slate-700/70">
                                        <div className="flex flex-col min-w-0">
                                            <a href={`/products/${item.slug}`} className="text-sm font-medium text-amber-50 truncate hover:text-amber-400" onClick={() => setIsOpen(false)}>
                                                {item.name}
                                            </a>
                                        ...
                                        <span className="text-sm font-semibold text-amber-400 ml-4 flex-shrink-0">
                            ...
                            <div className="p-4 border-t border-slate-700 bg-slate-700/50">
                                ...
                                <button
                                    ...
                                    className="w-full bg-amber-600 hover:bg-amber-500 text-white font-bold py-2 rounded-lg transition duration-200"
                                >
                                    Ir a Pagar
                                </button>
                            </div>
